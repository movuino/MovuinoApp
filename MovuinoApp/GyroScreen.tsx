...
import React, { useEffect, useState, FunctionComponent, useContext, useLayoutEffect,useRef } from "react";
import {
	View,
	StyleSheet,
	Text,
...
import { Characteristic, BleError } from "react-native-ble-plx";
...
type SensorScreenProps = NativeStackScreenProps<SensorsStackParamList, "Gyro">;
...
const SensorScreen: FunctionComponent<SensorScreenProps> = (props) => {
    ...
    useEffect(() => {
        deviceConnected?.characteristicsForService(props.route.params.serviceUUID).then(chars => {chars[0].monitor(onUpdate)})
	...
	return (
		<View style={styles.container}>
            {isPortrait() && 
            <View>
                <ScreenHeader title={"Gyroscope"} showSettings={false} onBack={props.navigation.goBack}></ScreenHeader>
